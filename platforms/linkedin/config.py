...
browser = ["Chrome"]
# Credentials: config_secrets.py (see config_secrets.py.example)
email = ""
password = ""

#PRO FEATURE - Optional! run browser in headless mode, no browser screen will be shown it will work in background.
# headless=True — стабильнее в systemd user service (без привязки к :0)
headless = True
...
# Отдельный профиль бота (не incognito каждый цикл)
chromeProfilePath = r"data/chrome-bot-profile/Default"
...
#salary - empty = no USD salary filter (better for EU)
salary = []
