...
DEFAULT_CONTACTS: dict[str, str] = {
    "telegram_username": "ilyailyailya27",
    "telegram_url": "https://t.me/ilyailyailya27",
    "github_url": "https://github.com/ilyasilkin27",
}
...
# Типовые отписки-заглушки ("посмотрим резюме и сами свяжемся, если подойдёт") —
# не вопрос и не приглашение к диалогу, отвечать там нечего. Слабая модель на
# такой короткой шаблонной фразе склонна тупо перефразировать её обратно
# работодателю, путая роли (наблюдался баг: бот писал "Рассмотрим нашу
# переписку..." как будто сам работодатель). Проще не отвечать вовсе.
_NO_REPLY_NEEDED_RE = re.compile(
    r"рассмотрим\s+(ваше|своё|наше)?\s*резюме.{0,120}?свяжемся\s+с\s+вами"
    r"|мы\s+с\s+вами\s+свяжемся"
    r"|свяжемся\s+с\s+вами,?\s+если",
    re.IGNORECASE | re.DOTALL,
)

# Явный отказ после скрининга ("к сожалению, нужен опыт с X") — без вопроса,
# отвечать нечего. Наблюдался баг: бот на такое сообщение переотвечал всей
# анкетой заново (повторно, слово в слово) — бессмысленно после того, как
# работодатель уже отказал. "?" в тексте исключает ложные срабатывания на
# "к сожалению, вакансия закрылась, но у нас есть другая — интересно?".
_DECLINE_AFTER_SCREENING_RE = re.compile(
    r"к\s+сожалению", re.IGNORECASE
)


def is_no_reply_needed(text: str) -> bool:
    """True, если последнее сообщение работодателя — типовая отписка без вопроса."""
    text = text or ""
    if _NO_REPLY_NEEDED_RE.search(text):
        return True
    if _DECLINE_AFTER_SCREENING_RE.search(text) and "?" not in text:
        return True
    return False
...
def build_reply_ai_query(
    *,
    ...
    return (
        ...
        "В истории переписки строки «Работодатель:» — это чужие слова, а "
        "не твои. Никогда не пересказывай и не перефразируй последнее "
        "сообщение работодателя от своего лица (например, если он написал "
        "«рассмотрим ваше резюме», не пиши в ответ «рассмотрим наше…»). "
        "Отвечай от первого лица как кандидат, по существу. "
